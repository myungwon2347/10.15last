...
FITSOFT['REST_API'] = {
    ...
    */
        ...
        // 서버로 전송 할 데이터 목록 (선택)
        var params = empty(res['params']) ? g_req[api_name] : res['params'];
        // var params = (empty(g_req) || empty(g_req[api_name])) ? res['params'] : g_req[api_name];
...
FITSOFT['IMAGE'] = {
    ...
    {// 파일 객체 변경 이벤트 (2020.07.24 / By.Chungwon)
        ...
        var format = empty(image_info['format']) ? ['jpg','jpeg','png','gif','bmp','BMP','JPG','JPEG','PNG','GIF', 'webp', 'WEBP', 'jfif', 'heic', 'JFIF', 'HEIC'] : image_info['format'];
        var size = empty(image_info) ? 20 : image_info['file_size'];
...
{// 데이터 관리 컨테이너 함수 (2021.06.18 / By.Chungwon)
    // main_idx 필수!!!!!!!!!!!!
    // main_idx는 검색결과 아이템 idx입니다.

/***** 초기화 *****/
    if(empty(api_name))
    {
        alert("not find api_name in getlistToDataStr");
        return;
    }
...
{// 3차원 리스트 등록 (2021.09.29 / By.Chungwon)
...
{// 에디터 및 파일처리 (2021.07.17 / By.Chungwon)
    ...
    /*************************** 3차원 리스트 등록 ***************************/ 
    // for(var key in g_multi_list)
    // {
    //     // 추가된 전체 데이터 목록 (삭제 반영 X)
    //     var insert_list = g_multi_list[key]['insert'];
    //     // 화면에 추가된 데이터 목록 (삭제 반영 O)
    //     var item_list = $("#list-" + key).find('.item-data');
    //     // 실제로 전송할 idx 목록
    //     var real_item_list = [];
    ...
    //     g_multi_list[key]['insert'] = real_item_list;
    //     g_multi_list[key]['origin'] = [];
    // }
    // params.append("multi_list", JSON.stringify(g_multi_list));
    params.append("multi_list", JSON.stringify(setMultiParams(g_multi_list)));
    /*************************** 3차원 리스트 등록 끝 ***************************/ 
...
{// 파일 객체 변경 이벤트 (2021.07.17 / By.Chungwon)
    ...
    var is_image = empty(params['is_image']) ? true : params['is_image'];
    ...
    $(file_list).each(function(i, file){
        ...
        {// 그 외 파일인 경우
            // [재정의] 수정과 추가를 구분하기 위해 
            file['idx'] = null;
            file['ref_key'] = image_key;
            file['o_name'] = file['name'];
            file['hit'] = 0;
            file['order_num'] = file['order_num'];
            
            // 리턴 변수에 file 넣기
            file_info_list.push(file);
...
function numberCounting(obj, val, durationVal)
{// 숫자 자동 증가
...
function hrefUrl(key, value)
{// URL로 페이지 이동 (페이지 전환 X) - 2020.09.23 / by.Chungwon

    // 키 파싱
    key = "&" + key + "=";

    // 정규식
    var reg_exp = new RegExp("\\" + key + "([a-z]+)", "gi");

    // 현재 페이지 정보 얻기
    var current_url = location.search === "" ? location.href + "?" : location.href;

    // 기존의 키/값 지우기
    current_url = current_url.replace(reg_exp,'');
...
function createSlick(selector)
{// 슬릭 연동 (2020.07.25 / By.Chungwon)
    selector.slick({
        // slidesToShow: 8,
        slidesToScroll: 1,
        autoplay: true,
