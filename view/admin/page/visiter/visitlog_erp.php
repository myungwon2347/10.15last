<?php 
    ...
    $gp_GVI = array(
        ...
        "user_name"               =>   paramVaildCheck("user_name", ""),
    ...
    // 검색 옵션 목록 도출
    $device_list = VisiterErp::getOptionList('device_type');
    $device_name_list = VisiterErp::getOptionList('device_name');
    $browser_list = VisiterErp::getOptionList('browser');
    $media_list = VisiterErp::getActionList();
?>
<!------------------------------------------------------- STYLE ----------------------------------------------------------->
<!------------------------------------------------------- STYLE ----------------------------------------------------------->
<style>
    ...
    .cont-search_board .search-item {display:flex; margin-bottom:6px; /* width:50%; */}
    ...
    .cont-search_board .search-item input[type='text'].event-search_keyword {width:20%; padding:5px; border:1px solid #d5d5d5; border-radius:5px; font-size : 0.9rem; margin-right:3px;}
    .cont-search_board .search-item input[type='text'].join-date {width:20%; font-size : 0.9rem;}
    .cont-search_board .search-item select {width:20%; margin-right:5px;}
    .cont-search_board .search-item #search-start_date {margin-right:5px;}
    .cont-search_board .search-item .search-item-tit {width:100px; font-weight:500;}
    .cont-search_board .search-item .btn-wrap {padding:0 10px; color:#fff; background-color:#0d2ea3; border-radius:4px; cursor:pointer; font-size:.9rem;}
    ...
    .table .table-frame.td .tr .tr-item:nth-child(7) {display:inline-block; text-align:unset; justify-content : unset; padding-left :30px;}
    ...
    <div class='cont'>
        <div class='cont-search_board'>
            ...
            <form class='search-item search-text search-form' data-event_type='submit' onsubmit='return false;'>
                <p class='search-item-tit'>NAME</p>
                <input class='event-search_keyword' type='text' data-key='user_name' placeholder="검색 할 이름을 입력하세요." />
        ...
        <div class='log-cont cont-board'>
            <div class="board-top">  
                ...
                <!-- <p class='cont-excel'>
                    <a class='btn' href='#' download='' onclick="downloadExcel(this, 'test', 'list-table');"><i class='xi-download'></i> 엑셀 다운로드</a>
                </p> -->
            </div>
            <div class='table'>
                <div class='table-frame board_column th'>
                    <div class='tr board_post'>
                        ...
                        <div class='tr-item'>
                            NAME
                            <span class='data-filter sort-item' data-key='user_name' data-event_type='click'>                                
                                <i class='xi-align-justify'></i>
                                <i class='xi-sort-desc'></i>
                                <i class='xi-sort-asc'></i>
                            </span>
                        ...
                        <div class='tr-item' style='justify-content:flex-start;'>
                            유입 경로
...
/**************************************************** GLOBAL VARIABLE VALUE DEFINED END ***********************************/
    ...
    function getVisitInfoAll(opt){
        ...
        /* API 호출 */
        sendAPI("/marketing/visiter_erp", "getVisitInfoAll", temp_params, function(res){
            // HTML 관련 변수
    ...
    function createHTMLVisitLog(log){
        ...
        if(referer !== "Access by URL")
        {
            try{
                ...
                else if(log['current_url'].indexOf('mk=gdn') !== -1)
                {// 구글 GDN 검색인 경우
                    network = decodeURIComponent(getStrAmong(log['current_url'], 'network=', '&'));
                    keyword = decodeURIComponent(getStrAmong(log['current_url'], 'keyword=', '&'));
                ...
                else if(log['current_url'].indexOf('mk=iss') !== -1)
                {// 인스타 스폰서 광고인 경우
                    network = decodeURIComponent(getStrAmong(log['current_url'], 'utm_medium=', '&'));
                    keyword = decodeURIComponent(getStrAmong(log['current_url'], 'utm_campaign=', '&'));
